'''
Starter Code for Problem 1 of HW4 Tufts COMP 150 Bayesian Deep Learning Fall 2019
...
Sample Usage
------------
To train an autoencoder (AE) on MNIST for 3 epochs with learning rate 0.01
...
from hw4starter_vae import (
    VariationalAutoencoder, eval_model_on_data, parse_args_from_stdin_using_common_format, print_dataset_info)
...
class Autoencoder(nn.Module):
    ...
    def calc_bce_loss(self, xbin_ND):
        ...
        xproba_ND : tensor shaped like xbin_ND, values within unit interval (0,1)
        ...
        e = .0000000001
        bce_loss = -torch.sum(xbin_ND * torch.log(xproba_ND + e) + (1.0 - xbin_ND) * torch.log(1.0-e-xproba_ND))
...
def train_for_one_epoch_of_gradient_update_steps(
    ...
    for batch_idx, (batch_data, _) in enumerate(train_loader):
        ...
        batch_loss_tensor, batch_xproba_ND = model.calc_bce_loss(batch_x_ND)
        ...
        if np.isnan(batch_loss_val):
            ...
            # Increment the total loss (sum across all batches)
...
if __name__ == "__main__":
    args = parse_args_from_stdin_using_common_format(
        nickname="AE",
        description="Autoencoder Example on MNIST Data")
    ## Use the local cpu
    ...
    model = Autoencoder(hidden_layer_sizes=args.hidden_layer_sizes)    
    ...
    for epoch in range(args.n_epochs + 1):
        ...
        ## Create a VAE from an AE
        # This chunk will copy all encoder/decoder parameters
        # from our latest AE model into a new 'VAE' model
        ...
        tr_loss, tr_l1, tr_bce = eval_model_on_data(tmp_vae_model, train_loader, device, args)
        print('  epoch %3d  train loss %.3f  bce %.3f  l1 %.3f' % (epoch, tr_loss, tr_bce, tr_l1))
        te_loss, te_l1, te_bce = eval_model_on_data(tmp_vae_model, test_loader, device, args)
