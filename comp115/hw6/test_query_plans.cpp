...
static void test_q2_table_scan()
{
    ...
    Iterator* q2 = project(select(nested_loops_join(nested_loops_join(table_scan(user), {0}, table_scan(routing), {0}), {4}, table_scan(message), {0}), q2_predicate), {5});
    CHECK(match(c2, q2));
    delete q2;
    delete c2;
}
...
static void test_q2_index_scan()
{
    ...
    Iterator* q2 = table_scan(control2);
    CHECK(match(c2, q2));
    delete q2;
    delete c2;
}
...
static void test_q3()
{
    ...
    Iterator *q3 = project(select(nested_loops_join(table_scan(user), {0}, table_scan(routing), {1}), q3_predicate), {1});
...
static void test_q4()
{
    ...
    Row froglet({"Froglet"});
    Iterator* q4 = table_scan(control4);
