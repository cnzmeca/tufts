...
static void union_compatible_different_names()
{
    Table* r = Database::new_table("r", ColumnNames{"a", "b"});
...
static void intersect_compatible_different_names()
{
    Table* r = Database::new_table("r", ColumnNames{"a", "b"});
    Table* s = Database::new_table("s", ColumnNames{"c", "d"});
    try {
...
static void diff_compatible_different_names()
{
    Table* r = Database::new_table("r", ColumnNames{"a", "b"});
    Table* s = Database::new_table("s", ColumnNames{"c", "d"});
    try {
...
static void rename_no_such_column()
{
    Table* r = Database::new_table("r", ColumnNames{"a", "b"});
    try {
...
static void rename_too_few_columns()
{
    Table* r = Database::new_table("r", ColumnNames{"a", "b"});
    try {
        rename(r, NameMap({{"a", "x"}}));
        fail();
    } catch (TableException& e) {
        // expected
    }
...
void test_relational_algebra(int argc, const char** argv)
{
    ...
    ADD_TEST(union_compatible_different_names);
    ...
    ADD_TEST(intersect_compatible_different_names);
    ...
    ADD_TEST(diff_compatible_different_names);
    ...
    ADD_TEST(rename_no_such_column);
    ADD_TEST(rename_too_few_columns);
    ADD_TEST(rename_ok);
