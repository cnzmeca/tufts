...
static void test_q2()
{
    NameMap names = NameMap({{"from_user_id", "user_id"},
                             {"send_date", "send_date"}});
    Table *q2 = project(select(join(rename(project(join(routing, message), ColumnNames{"from_user_id", "send_date"}), names), user), q2_predicate), ColumnNames{"send_date"});
...
static void test_q3()
{
    NameMap names = NameMap({{"to_user_id", "user_id"},
                             {"send_date", "send_date"}});
    Table *q3 = project(select(join(rename(project(join(routing, message), ColumnNames{"to_user_id", "send_date"}), names), user), q3_predicate), ColumnNames{"username"});
...
{
    return row->value("from_username") == "Unguiferous";
}

static bool q4_to_predicate(Row *row)
{
    return row->value("to_username") == "Froglet";
}

static void test_q4()
{
    NameMap names1 = NameMap({{"user_id", "from_user_id"},
                             {"username", "from_username"},
                             {"birth_date", "birth_date"}});
    NameMap names2 = NameMap({{"user_id", "to_user_id"},
                             {"username", "to_username"},
                             {"birth_date", "birth_date"}});
    Table *from = select(join(project(rename(user, names1), ColumnNames{"from_user_id", "from_username"}), routing), q4_from_predicate);
    Table *to = select(join(project(rename(user, names2), ColumnNames{"to_user_id", "to_username"}), routing), q4_to_predicate);
    Table *q4 = project(join(message, join(from, to)), ColumnNames{"send_date"});
...
static void test_q5()
{
    NameMap names = NameMap({{"user_id", "from_user_id"},
                             {"username", "username"},
                             {"birth_date", "birth_date"}});
    Table *q5 = project(join(join(routing, select(message, q5_predicate)), rename(user, names)), ColumnNames{"username"});
...
{
    return row->value("from_username") == "Bamboozled";
}
...
static void test_q6()
{
    ...
    NameMap names2 = NameMap({{"user_id", "to_user_id"},
                             {"username", "to_username"}});
    ColumnNames cols = ColumnNames{"user_id", "username"};
    Table *q6 = project(select(join(rename(project(user, cols), names2), join(rename(project(user, cols), names1), routing)), q6_predicate), ColumnNames{"to_username"});
...
static void test_q7()
{
    ...
    NameMap names2 = NameMap({{"user_id", "to_user_id"},
                             {"username", "to_username"}});
    ColumnNames cols = ColumnNames{"user_id", "username"};
    Table *q7 = project(join(rename(project(user, cols), names2), join(rename(project(user, cols), names1), join(select(message, q7_predicate), routing))), ColumnNames{"from_username", "to_username"});
