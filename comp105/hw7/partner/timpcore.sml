...
(* function [[ty]], to check the type of an expression, given $\itenvs$ ((prototype)) 417 *)
        | ty (AAT (a, i))        = 
            let val tau_a = ty a
                val tau_i = ty i 
            in  if eqType (tau_i, INTTY) then
                    case tau_a of
                        ARRAYTY(tau) => tau
                        | _ => raise TypeError ("Non-array")
                else
                    raise TypeError ("Index is not of type INTTY")
            end
        | ty (APUT (a, i, e))    = 
            let val tau_a = ty a
                val tau_i = ty i
                val tau_e = ty e
            in  if eqType (tau_i, INTTY) then
                    if eqType (tau_a, ARRAYTY(tau_e)) then
                        tau_e
                    else
                        raise TypeError ("Array and exp are of different types")
                else raise TypeError ("Index is not of type INTTY")
            end
        | ty (AMAKE (len, init)) = 
            let val tau_len = ty len
                val tau_init = ty init
            in  if eqType (tau_len, INTTY) then
                    ARRAYTY(tau_init)
                else
                    raise TypeError ("Length is not of type INTTY")
            end
        | ty (ASIZE a)           =
            let val tau_a = ty a
            in  case tau_a of
