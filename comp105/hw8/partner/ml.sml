...
fun solve TRIVIAL = idsubst
  | solve ((TYVAR a) ~ t2) =
        if (List.foldr (fn (x, y) => a = x orelse y) false (freetyvars t2)) then
            if (eqType (TYVAR(a), t2)) then
                idsubst
            else
                raise TypeError "No sol."
        else
            (a |--> t2)
  | solve (t1 ~ (TYVAR a)) = solve ((TYVAR a) ~ t1)
  | solve ((TYCON a1) ~ (TYCON a2)) = 
        if (eqType ((TYCON a1), (TYCON a2))) then
            idsubst
        else
            raise TypeError "No sol."
  | solve ((CONAPP (t1, t1s)) ~ (CONAPP (t2, t2s))) =
        let val t = (t1 ~ t2)
            val ts = (ListPair.foldrEq (fn (x, y, z) => (x ~ y) /\ z) TRIVIAL
                (t1s, t2s)) handle UnequalLengths => raise TypeError "No sol."
        in  (solve (t /\ ts))
        end
  | solve (_ ~ _) = raise TypeError "No sol."
  | solve (c1 /\ c2) =
...
val () = Unit.checkAssert "trivial"
         (fn () => hasSolution TRIVIAL)

val () = Unit.checkAssert "tyvar ~ tyvar"
         (fn () => hasSolution (alpha ~ beta))

val () = Unit.checkAssert "tyvar ~ tycon"
         (fn () => hasSolution (alpha ~ booltype))

val () = Unit.checkAssert "tyvar ~ conapp"
         (fn () => hasSolution (alpha ~ (listtype booltype)))

val () = Unit.checkAssert "tycon ~ tyvar"
         (fn () => hasSolution (booltype ~ beta))

val () = Unit.checkAssert "tycon ~ tycon"
         (fn () => hasSolution (booltype ~ booltype))

val () = Unit.checkAssert "tycon ~ conapp"
         (fn () => hasNoSolution (booltype ~ (listtype booltype)))

val () = Unit.checkAssert "conapp ~ tyvar"
         (fn () => hasSolution ((listtype booltype) ~ beta))

val () = Unit.checkAssert "conapp ~ tycon"
         (fn () => hasNoSolution ((listtype booltype) ~ booltype))

val () = Unit.checkAssert "conapp ~ conapp"
         (fn () => hasSolution ((listtype booltype) ~ (listtype booltype)))

val () = Unit.checkAssert "c and c"
         (fn () => hasSolution ((alpha ~ alpha) /\ (beta ~ beta)))
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 523b *)
      fun literal (BOOLV b) = (booltype, TRIVIAL)
        | literal (NUM n) = (inttype, TRIVIAL)
        | literal (SYM s) = (symtype, TRIVIAL)
        | literal NIL = ((listtype alpha), TRIVIAL)
        | literal (PAIR (b, bs)) = 
            let val (t, c) = typeof ((LITERAL b), Gamma)
                val (ts, cs) = typeof ((LITERAL bs), Gamma)
            in  ((listtype t), (c /\ cs /\ ((listtype t) ~ ts)))
            end
        | literal _ = raise BugInTypeInference "No closure or primitives"
...
(* function [[ty]], to infer the type of a \nml\ expression, given [[Gamma]] 523a *)
      fun ty (LITERAL n) = literal n
        ...
        | ty (LAMBDA (formals, body)) =
            let val ts = (foldr (fn (x, y) =>
                (freshtyvar () :: y)) [] formals)
                val Gamma' = (ListPair.foldr (fn (x, y, z) => bindtyscheme
                (x, (FORALL ([], y)), z)) Gamma (formals, ts))
                val (t, c) = typeof (body, Gamma')
            in (funtype (ts, t), c)
            end
        | ty (LETX (LET, bs, body))    =
            let val (ns, es) = ListPair.unzip bs
                val (ts, cs) = typesof (es, Gamma)
                val Gamma' = (ListPair.foldr (fn (x, y, z) => bindtyscheme
                (x, generalize (y, freetyvarsGamma z), z)) Gamma (ns, ts))
            in typeof (body, Gamma')
            end
          ...
          ((* primitives for \nml\ and \uml\ [[::]] 527e *)
                          ...
                          ("cdr",   unaryOp  (fn (PAIR (_, cdr)) => cdr 
                                               ...
                                               | _   => raise BugInTypeInference
                                                  ...
                                                  funtype ([listtype alpha],
                                                             listtype alpha)) ::
            ("fst", unaryOp (fn (PAIR (x, _)) => x
                        | _ => raise BugInTypeInference "non-pair"),
                        funtype ([pairtype (alpha, beta)], alpha)) ::
            ("snd", unaryOp (fn (PAIR (_, y)) => y
                        | _ => raise BugInTypeInference "non-pair"),
                        funtype ([pairtype (alpha, beta)], beta)) :: 
            ("pair", binaryOp (fn (x, y) => PAIR (x, y)),
