...
fun freeIn (LITERAL(_)) n = false
  ...
  | freeIn (BEGIN(es)) n = (foldr (fn (x, y) => (freeIn x n) orelse y) false es)
  | freeIn (APPLY(e, es)) n = ((freeIn e n) orelse
                         (foldr (fn (x, y) => (freeIn x n) orelse y) false es)) 
  | freeIn (LETX(_, [], e)) n = freeIn e n
  | freeIn (LETX(LET, b::bs, e)) n = ((freeIn (snd b) n) orelse
                                     (freeIn (LETX(LET, [b], e)) n))
  | freeIn (LETX(LETSTAR, b::bs, e)) n = freeIn (LETX (LET, [b],
                                              LETX (LETSTAR, bs, e))) n
  | freeIn (LETX(LETREC, b::bs, e)) n = if (not (n = (fst b))) then
                        freeIn (LETX(LETREC, [b], e)) n else false
  | freeIn (LAMBDA([], e)) n = freeIn e n
  | freeIn (LAMBDA(b::bs, e)) n = if (not (n = b)) then
                                (freeIn (LAMBDA(bs, e)) n) else false

        val () = Unit.checkExpectWith Bool.toString "true"
            (fn () => (freeIn (LAMBDA (["y"], (APPLY ((VAR "equals?"),
                                        [(VAR "x"), (VAR "y")])))) "x")) true
        val () = Unit.checkExpectWith Bool.toString "false"
            (fn () => (freeIn (LAMBDA (["y"], (APPLY ((VAR "equals?"), 
                                        [(VAR "x"), (VAR "y")])))) "y")) false
        val () = Unit.reportWhenFailures ()

fun improve ((xs, e), env) = ((xs, e), (List.filter 
                                        (fn (y) => freeIn e (fst y)) env))
