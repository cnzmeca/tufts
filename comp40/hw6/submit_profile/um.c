...
// MAX = 2^32 (smallest number that can't be stored in uint32_t)
...
int main(int argc, char *argv[])
{
        ...
        while (1) {
                ...
                if (op == 13) {
                        ...
                        *a = *(Seq_get(um->mem, *b) + *c + 1);  
                } else if (op == 2) {
                        *(Seq_get(um->mem, *a) + *b + 1) = *c;
                } else if (op == 6) {
                        *a = ~(*b & *c);
                } else if (op == 3) {
                        *a = (*b + *c) % MAX;
                } else if (op == 12) {
                        *pc = Segment_move(um->mem, um->unused_ids, b, c);
                } else if (op == 0) {
                        if (*c != 0) {
                                *a = *b;
                        }
